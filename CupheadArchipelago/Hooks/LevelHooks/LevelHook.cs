...
namespace CupheadArchipelago.Hooks.LevelHooks {
    internal class LevelHook {
            ...
            static void Postfix(Level __instance) {
                ...
                if (APData.IsCurrentSlotEnabled()) {
                    ...
                    if (!APData.CurrentSData.IsOverridden(Overrides.NoDataStorageOverride)) {
                        APDataStorage.WriteCurrentLevel(lv);
                    }
        ...
        internal static class _OnPreWin {
            private static readonly HashSet<Levels> secretLevels = [
                Levels.Veggies,
                Levels.FlyingGenie,
                        ...
                        switch (instance.LevelType) {
                            case Level.Type.Battle: {
                                    ...
                                    if (Level.Difficulty >= battleNormalMode) {
                                        ...
                                        Logging.Log($"[LevelHook] Secret: {(vsecret?"Yes":"No")}");
                                        int ccheck = 0;
        ...
        internal static class _OnLose {
            static bool Prefix(Level __instance, bool ___secretTriggered) {
                if (APData.IsCurrentSlotEnabled() && APSettings.DeathLink == DeathLinkMode.Normal && APClient.IsDeathLinkActive()) {
                    DeathLinkMngr.TriggerSendDeathLink(__instance);
                }
                return true;
            }
        }
